...
export function ConfirmDialog({
  ...
  const variantStyles = {
    ...
    info: 'bg-white text-slate-900 hover:bg-white/90 shadow-[0_0_20px_rgba(255,255,255,0.2)]',
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 cursor-default">
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.9)] overflow-hidden animate-in zoom-in-95 duration-300 ease-out">
        {/* Línea de acento superior */}
        <div className={`h-1.5 w-full ${
          variant === 'danger' ? 'bg-red-500' : 
          variant === 'warning' ? 'bg-amber-500' : 
          variant === 'success' ? 'bg-emerald-500' : 'bg-white'
        }`} />
        ...
        <div className="p-10">
          <div className="flex flex-col items-center text-center space-y-6">
            <div className={`p-5 rounded-[25px] ${
              ...
              variant === 'success' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-white/10 text-white'
            }`}>
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm font-medium text-white/50 leading-relaxed max-w-sm mx-auto">
                {description}
              </p>
            </div>
          </div>
